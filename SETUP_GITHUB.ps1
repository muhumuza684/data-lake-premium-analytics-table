...
param(
  ...
  [string]$ProjectPath = "C:\PowerBI\INFORIVAPREM",
  [switch]$CreateRemote,
  [switch]$Push
)
$ErrorActionPreference = "Stop"
$RepoName = "INFORIVAPREM-powerbi-visual"
$ExpectedName = "INFORIVAPREM"
...
if (-not (git diff --cached --quiet)) { git commit -m "Initial INFORIVAPREM Power BI visual" }
...
if ($CreateRemote) {
  ...
  if ($LASTEXITCODE -ne 0) { gh repo create "$Owner/$RepoName" --public --description "Independent INFORIVAPREM Power BI custom visual" --source . --remote origin --push }
...
if ($Push) {
  ...
  if ($PSCmdlet.ShouldProcess("origin/main", "Push INFORIVAPREM")) { git push -u origin main }
}
Write-Host "Prepared INFORIVAPREM. Remote creation/push are opt-in switches." -ForegroundColor Green
